...
image: /images/post/common-frontend-interview-questions/interview.webp
...
> (2023/5/22 更新) Web API 有一個叫做[structuredClone](https://developer.mozilla.org/en-US/docs/Web/API/structuredClone)的 API，已經可以實現 deepClone 的功能，但是各家的瀏覽器都是近幾個版本才支援的，所以如果有舊版本瀏覽器的話，要做好兼容性。
...
- 監聽 resize 事件時，不用每次變動就立即執行 function，只要最後停止時在做計算即可

- 以 Search bar 的功能來說，不用使用者每輸入一個字都去 call API，只要最後停下來後再去要資料就好了
