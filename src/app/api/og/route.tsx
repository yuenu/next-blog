...
export async function GET(req: Request) {
  try {
    ...
    const heading =
      values.heading.length > maxTextLength
        ? `${values.heading.substring(0, maxTextLength)}...`
        : values.heading

    const { mode } = values
    const paint = mode === 'dark' ? '#fff' : '#000'

    const fontSize = heading.length > maxTextLength ? '60px' : '80px'
        ...
        >
          ...
          <div tw="mt-auto flex justify-between w-full text-xl">
            ...
            <div tw="flex items-center">
              ...
              <div tw="flex ml-2">github.com/yuenu</div>
            </div>
          </div>
        </div>
      ),
